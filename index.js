...
app.get("/callback", async (req, res) => {
  ...
  try {
    ...
    res.send(`
      ...
      <a href="/create-playlist">Click here to generate playlist from your top tracks</a>
    `);
  } catch (err) {
...
app.get("/create-playlist", async (req, res) => {
  try {
    ...
    const trackURIs = topTracks.data.items.map((track) => track.uri);

    // Placeholder for ML model — you can plug in custom logic here
    const recommendedURIs = trackURIs; // For now, just reuse the same

    // Step 3: Create a new playlist
    const playlist = await axios.post(
      `https://api.spotify.com/v1/users/${user_id}/playlists`,
      {
        name: "RecomML Playlist",
        description: "Generated by RecomML based on your top tracks",
        public: true,
      },
      {
        headers: { Authorization: `Bearer ${access_token}` },
